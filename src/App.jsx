...
function Dashboard({ user }) {
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-white font-sans pb-24 overflow-x-hidden selection:bg-pink-500/30">
      ...
      <main className="w-full max-w-7xl mx-auto p-2 sm:p-4 lg:p-8 grid grid-cols-1 md:grid-cols-12 gap-4 sm:gap-8 mt-2 sm:mt-4">
        ...
        <div className="md:col-span-6 lg:col-span-5 bg-neutral-900/40 border border-neutral-800 rounded-3xl p-4 sm:p-6 md:h-[80vh] flex flex-col">
              ...
              <div key={ficha.id} className={`bg-neutral-950 border ${ficha.enEspera ? 'border-orange-500/30' : 'border-neutral-800'} p-4 rounded-2xl flex items-center justify-between gap-3 shadow-[0_4px_15px_rgba(0,0,0,0.5)] hover:border-neutral-700 transition-colors`}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {ficha.estado === 'Pendiente' ? <span className="bg-orange-400/10 text-orange-400 text-[10px] px-2 py-0.5 rounded-full uppercase font-bold border border-orange-400/20">Pendiente</span> :
                      ficha.estado === 'Anulada' ? <span className="bg-red-400/10 text-red-400 text-[10px] px-2 py-0.5 rounded-full uppercase font-bold border border-red-400/20">Anulada</span> :
                        ficha.estado === 'Devuelta' ? <span className="bg-neutral-500/20 text-neutral-400 text-[10px] px-2 py-0.5 rounded-full uppercase font-bold border border-neutral-500/30">Devuelta</span> :
                ...
                <div className="flex items-center gap-2">
                  ...
                  {ficha.comprobanteUrl && (
                    <button onClick={() => setViewingImage(ficha.comprobanteUrl)} className="p-2 sm:p-3 bg-purple-500/10 text-purple-400 rounded-xl hover:bg-purple-500 hover:text-white border border-purple-500/20" title="Ver Comprobante"><Eye size={18} /></button>
                  )}
                  {ficha.estado !== 'Devuelta' && ficha.estado !== 'Anulada' && (
                    <button onClick={() => setFichaToReturn(ficha.id)} className="p-2 sm:p-3 bg-neutral-500/10 text-neutral-400 rounded-xl hover:bg-neutral-500 hover:text-white border border-neutral-500/20" title="Devolver Ficha"><LogOut size={18} className="rotate-180" /></button>
                  )}
                  <button onClick={() => setFichaToDelete(ficha.id)} className="p-2 sm:p-3 bg-red-500/10 text-red-400 rounded-xl hover:bg-red-500 hover:text-white border border-red-500/20" title="Eliminar"><Trash2 size={18} /></button>
                </div>
              </div>
            ))}
          </div>
      ...
      {fichaToDelete && (
        <div className="fixed inset-0 bg-black/90 z-[80] flex flex-col items-center justify-center p-4">
          <div className="w-full max-w-sm bg-neutral-900 border border-red-500/50 rounded-3xl p-6 text-center">
            ...
            <p className="text-neutral-400 text-sm mb-6">Esto regresará un número al conteo de fichas disponibles.</p>
            <div className="flex gap-3">
              ...
              <button onClick={confirmarEliminarFicha} className="flex-1 py-3 rounded-xl bg-red-500 text-white font-bold uppercase">Eliminar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {fichaToReturn && (
        <div className="fixed inset-0 bg-black/90 z-[80] flex flex-col items-center justify-center p-4">
          <div className="w-full max-w-sm bg-neutral-900 border border-neutral-500/50 rounded-3xl p-6 text-center">
            <LogOut className="text-neutral-400 mx-auto mb-4 rotate-180" size={40} />
            <h3 className="text-xl font-bold text-white mb-2 uppercase">¿Devolver Ficha?</h3>
            <p className="text-neutral-400 text-sm mb-6">La ficha quedará marcada como Devuelta y su número retornará al conteo disponible.</p>
            <div className="flex gap-3">
              <button onClick={() => setFichaToReturn(null)} className="flex-1 py-3 rounded-xl border border-neutral-700 text-neutral-300">Cancelar</button>
              <button onClick={confirmarDevolverFicha} className="flex-1 py-3 rounded-xl bg-neutral-500 text-white font-bold uppercase">Devolver</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className={`fixed top-4 right-4 z-[110] max-w-sm w-[90%] p-4 rounded-2xl shadow-2xl animate-in slide-in-from-top-4 fade-in duration-300 border backdrop-blur-xl ${
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-xs font-medium text-white/90">{toastMessage.message}</p>
            </div>
